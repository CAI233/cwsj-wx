...
import com.szkj.platform.busiz.domain.WorksTag;
import com.szkj.platform.busiz.service.TagService;
import com.szkj.platform.busiz.service.WorksTagService;
...
@Controller
@RequestMapping(value = "/api/busiz/works")
public class WorksTagController {

    @Autowired
    private WorksTagService worksTagService;
    ...
    public Object getList(HttpServletRequest request, @RequestBody SortCondition bean) {
        ...
        try {
            ...
            Object result = worksTagService.getList(bean, sort);
    ...
    @ResponseBody
    public Object save(HttpServletRequest request, @RequestBody WorksTag bean) {
        SysUser user = CheckUserHelper.checkUserInfo(request.getHeader("token"));
        if (user == null) {
            String message = MessageAPi.getMessage(MsgCodeEnum.TOKEN_FAILED.code());
            return JsonResult.getExpire(message);
        ...
        try {
            ...
            if (bean.getTag_id() == null) {
                //新增
                WorksTag tag = worksTagService.selectTagByName(bean.getTag_name());
                if (tag != null) {
                    return JsonResult.getError("标签名称已存在！");
                }
                worksTagService.saveTag(bean);
                jsonResult.setMessage(MessageAPi.getMessage(MsgCodeEnum.ADD_SUCCESS.code()));
                return jsonResult;
            } else {
                //修改
                WorksTag tag = worksTagService.selectTagByNameOutId(bean.getTag_name(), bean.getTag_id());
                if (tag != null) {
                    return JsonResult.getError("标签名称已存在！");
                }
                worksTagService.updateTag(bean);
    ...
    public Object del(HttpServletRequest request, @RequestBody IdsCondition bean) {
        ...
        try {
            ...
            worksTagService.delTag(ids_str);
