...
import com.szkj.platform.busiz.domain.BookChapter;
import com.szkj.platform.busiz.service.BookChapterService;
...
public class BookChapterController {
    ...
    public Object getTree(HttpServletRequest request,@RequestBody SortCondition bean){
        ...
        try{
           List<BookChapter> list = bookChapterService.getCatList(bean.getSearchText());
           //转换为树形结构
           List<BookChapter> result = bookChapterService.getTree(list);
    ...
    @ResponseBody
    public Object save(HttpServletRequest request, @RequestBody BookChapter bean){
        SysUser user = CheckUserHelper.checkUserInfo(request.getHeader("token"));
        if (user == null){
            String message = MessageAPi.getMessage(MsgCodeEnum.TOKEN_FAILED.code());
            return JsonResult.getExpire(message);
        ...
        try{
            ...
            if (bean.getChapter_id() == null){
                //新增
                BookChapter cat = bookChapterService.selectCatByName(bean.getChapter_name());
                if (cat != null){
                    return JsonResult.getError("分类名称已存在！");
                }
                bookChapterService.saveCat(bean);
                jsonResult.setMessage(MessageAPi.getMessage(MsgCodeEnum.ADD_SUCCESS.code()));
                return jsonResult;
            }else {
                //修改
                BookChapter cat = bookChapterService.selectCatByNameOutId(bean.getChapter_name(),bean.getChapter_id());
                if (cat != null){
                    return JsonResult.getError("分类名称已存在！");
                }
                bookChapterService.updateCat(bean);
    ...
    public Object del(HttpServletRequest request, @RequestBody IdsCondition bean){
        ...
        try{
            ...
            for (int i =0;i<bean.getIds().length;i++){
                ...
                bookChapterService.delCatByPath(cat_id);
            }
