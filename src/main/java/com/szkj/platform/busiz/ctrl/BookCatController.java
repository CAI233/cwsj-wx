...
import com.szkj.platform.busiz.domain.BookCat;
import com.szkj.platform.busiz.service.BookCatService;
...
public class BookCatController {
    ...
    public Object getTree(HttpServletRequest request,@RequestBody SortCondition bean){
        ...
        try{
           List<BookCat> list = bookCatService.getCatList(bean.getSearchText());
           //转换为树形结构
           List<BookCat> result = bookCatService.getTree(list);
    ...
    @ResponseBody
    public Object save(HttpServletRequest request, @RequestBody BookCat bean){
        SysUser user = CheckUserHelper.checkUserInfo(request.getHeader("token"));
        if (user == null){
            String message = MessageAPi.getMessage(MsgCodeEnum.TOKEN_FAILED.code());
            return JsonResult.getExpire(message);
        ...
        try{
            ...
            if (bean.getCat_id() == null){
                //新增
                BookCat cat = bookCatService.selectCatByName(bean.getCat_name());
                if (cat != null){
                    return JsonResult.getError("分类名称已存在！");
                }
                bookCatService.saveCat(bean);
                jsonResult.setMessage(MessageAPi.getMessage(MsgCodeEnum.ADD_SUCCESS.code()));
                return jsonResult;
            }else {
                //修改
                BookCat cat = bookCatService.selectCatByNameOutId(bean.getCat_name(),bean.getCat_id());
                if (cat != null){
                    return JsonResult.getError("分类名称已存在！");
                }
                bookCatService.updateCat(bean);
    ...
    public Object del(HttpServletRequest request, @RequestBody IdsCondition bean){
        ...
        try{
            ...
            for (int i =0;i<bean.getIds().length;i++){
                ...
                bookCatService.delCatByPath(cat_id);
            }
