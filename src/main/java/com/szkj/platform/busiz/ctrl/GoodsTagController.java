...
import com.szkj.platform.busiz.domain.GoodsTag;
import com.szkj.platform.busiz.service.GoodsTagService;
...
public class GoodsTagController {
    ...
    public Object getList(HttpServletRequest request , @RequestBody SortCondition bean){
        ...
        try{
           ...
           Object result = goodsTagService.getList(bean,sort);
    ...
    @ResponseBody
    public Object save(HttpServletRequest request, @RequestBody GoodsTag bean){
        SysUser user = CheckUserHelper.checkUserInfo(request.getHeader("token"));
        if (user == null){
            String message = MessageAPi.getMessage(MsgCodeEnum.TOKEN_FAILED.code());
            return JsonResult.getExpire(message);
        ...
        try{
            ...
            if (bean.getTag_id() == null){
                //新增
                GoodsTag tag = goodsTagService.selectTagByName(bean.getTag_name());
                if (tag != null){
                    return JsonResult.getError("标签名称已存在！");
                }
                goodsTagService.saveTag(bean);
                jsonResult.setMessage(MessageAPi.getMessage(MsgCodeEnum.ADD_SUCCESS.code()));
                return jsonResult;
            }else {
                //修改
                GoodsTag tag = goodsTagService.selectTagByNameOutId(bean.getTag_name(),bean.getTag_id());
                if (tag != null){
                    return JsonResult.getError("标签名称已存在！");
                }
                goodsTagService.updateTag(bean);
    ...
    public Object del(HttpServletRequest request, @RequestBody IdsCondition bean){
        ...
        try{
            ...
            goodsTagService.delTag(ids_str);
