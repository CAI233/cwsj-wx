...
import com.szkj.platform.busiz.domain.Cat;
...
public class CatController {
    ...
    public Object getTree(HttpServletRequest request,@RequestBody SortCondition bean){
        ...
        try{
           List<Cat> list = catService.getCatList(bean.getSearchText());
           //转换为树形结构
           List<Cat> result = catService.getTree(list);
    ...
    @ResponseBody
    public Object save(HttpServletRequest request, @RequestBody Cat bean){
        SysUser user = CheckUserHelper.checkUserInfo(request.getHeader("token"));
        if (user == null){
            String message = MessageAPi.getMessage(MsgCodeEnum.TOKEN_FAILED.code());
            return JsonResult.getExpire(message);
        ...
        try{
            ...
            if (bean.getCat_id() == null){
                //新增
                Cat cat = catService.selectCatByName(bean.getCat_name());
                if (cat != null){
                    return JsonResult.getError("分类名称已存在！");
                }
                catService.saveCat(bean);
                jsonResult.setMessage(MessageAPi.getMessage(MsgCodeEnum.ADD_SUCCESS.code()));
                return jsonResult;
            }else {
                //修改
                Cat cat = catService.selectCatByNameOutId(bean.getCat_name(),bean.getCat_id());
                if (cat != null){
                    return JsonResult.getError("分类名称已存在！");
                }
                catService.updateCat(bean);
    ...
    public Object del(HttpServletRequest request, @RequestBody IdsCondition bean){
        ...
        try{
            ...
            for (int i =0;i<bean.getIds().length;i++){
                ...
                catService.delCatByPath(cat_id);
            }
